...
exports.sendOTP = async (req, res) => {
  ...
  if (!_b.college_id || !_b.password) {
    ...
    try {
      ...
      console.log(_b.college_id, user);
      ...
      if (!user) {
        ...
        if (validPassword) {
          ...
          res.status(200).json({
            success: true,
            message:
              "OTP for verification has been sent to your registered college email.",
          });
...
exports.login = async (req, res) => {
  ...
  if (!_b.college_id || !_b.password) {
    ...
    try {
      ...
      }).select("college_id student_name password login_otp");
...
exports.registerAdmin = async (req, res) => {
  try {
    await User.create({
      college_id: "201801158",
      college_email: "201801158@daiict.ac.in",
      password: "123456",
      active: true,
      permission: "admin",
    });
...
exports.resetPassword = async (req, res) => {
  ...
  if (!_b.password || !_b.token) {
    ...
    try {
      ...
      const user = await User.findOne({ temporarytoken: _b.token }).select(
        "student_name college_email password temporarytoken"
      );
      ...
      if (!user) {
        ...
        const sendConfirmationMail = await Mailer.sendDM(
          user,
          "passwordUpdated"
        );
      }
...
exports.getOne = async (req, res) => {
  ...
  const user = await User.findOne({
    college_id: _p.college_id.toUpperCase(),
  }).lean();

  if (!user) {
    res.status(200).json({
      success: false,
      message: "Incorrect College ID. Please try again.",
    });
  } else {
...
exports.updateOne = (req, res) => {
  ...
  User.updateOne({ college_id: _b.college_id }, _b)
    .then((data) => {
      res
        .status(200)
...
exports.timeline = async (req, res) => {
  try {
    ...
    const companies = await Company.find({ passout_batch: user.passout_batch })
      .select("company_name candidates")
      .lean();
...
exports.changePassword = async (req, res) => {
  ...
  if (!_b.old_password || !_b.new_password) {
    res
      .status(200)
      .json({ success: false, message: "Old or new password is missing." });
  } else {
    try {
      const user = await User.findOne({
...
exports.contributions = (req, res) => {
  Interview.find({ author_id: req.decoded.college_id })
    .lean()
    .sort({ created_at: -1 })
    .then((interviews) => {
...
exports.achievements = (req, res) => {
  Placements.find({ student_college_id: req.decoded.college_id })
    .lean()
