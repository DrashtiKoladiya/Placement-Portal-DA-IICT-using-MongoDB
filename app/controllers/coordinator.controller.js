...
exports.add = async (req, res) => {
  ...
  if (_b.college_email.toLowerCase().split("@")[1] !== Constant.emailSuffix) {
    res
      .status(200)
      .json({ success: false, message: "Invalid DAIICT College Email ID." });
  } else {
    try {
      ...
      res
        .status(200)
        .json({ success: true, message: "Coordinator successfully added." });
...
exports.getAll = (req, res) => {
  User.find({ permission: { $in: Constant.placementTeamRoles } })
    .select(
      "student_name college_id college_email alternate_contact_no permission"
    )
    .lean()
    .then((coordinators) => {
